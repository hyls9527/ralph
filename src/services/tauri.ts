...
export const tauri = {
  ...
  getFavorites: () =>
    invoke<Array<{ fullName: string; evaluationJson: string }>>(
      'get_favorites',
    ),
  ...
  removeFavorite: (fullName: string) => invoke('remove_favorite', { fullName }),
  ...
  getEvaluationHistory: (repoFullName: string) =>
    ...
    >('get_evaluation_history', { repoFullName }),

  getTrendingRepos: () => invoke<SearchResponse>('get_trending_repos'),

  clearCache: () => invoke('clear_cache'),

  saveSettings: (token: string) => invoke('save_settings', { token }),

  generateBadge: (grade: string, score: number, repoFullName: string) =>
    invoke<BadgeInfo>('generate_badge', { grade, score, repoFullName }),

  startDiscovery: () => invoke<DiscoveryStatus>('start_discovery'),

  stopDiscovery: () => invoke<DiscoveryStatus>('stop_discovery'),
  ...
  clearDiscoveryResults: () => invoke('clear_discovery_results'),

  updateDiscoveryConfig: (config: DiscoveryConfig) =>
    invoke<DiscoveryConfig>('update_discovery_config', { config }),

  getDiscoveryConfig: () => invoke<DiscoveryConfig>('get_discovery_config'),
  ...
  getBatchSessions: () => invoke<BatchSession[]>('get_batch_sessions'),
