          ...
          {history.map((item, index) => (
            ...
            >
              <div className="flex items-center gap-2 min-w-0 flex-1">
                <svg className={`w-3.5 h-3.5 flex-shrink-0 ${isLight ? 'text-gray-400' : 'text-gray-500'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div className={`flex items-center gap-2 flex-shrink-0 ${isLight ? 'text-gray-400' : 'text-gray-500'}`}>
                <span>{item.resultCount > 0 ? `${item.resultCount} 结果` : '无结果'}</span>
