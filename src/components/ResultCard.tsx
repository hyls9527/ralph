...
const DimensionDrillDown: React.FC<{ dimension: { dimension: string; score: number; maxScore: number; subScores?: [string, number, number][] } }> = ({ dimension }) => {
  ...
  return (
    <div className="border-t border-gray-700/50 pt-1.5 mt-1.5">
      <button onClick={() => setOpen(!open)} className="text-xs text-gray-500 hover:text-gray-300 transition-colors flex items-center gap-1 w-full">
...
const ResultCard: React.FC<ResultCardProps> = ({ project, onDetailClick, isFavorite, onFavoriteToggle }) => {
  ...
  const handleShare = async () => {
    const shareText = `${repo.fullName} | Ralph 评定: ${totalScore.toFixed(1)}分 / ${grade}级 / ${trackLabels[track]}轨道 | 推荐指数: ${recommendationIndex.toFixed(2)} | ${repo.htmlUrl}`;
    try {
      ...
      <div className="p-5">
        ...
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="flex flex-col items-end gap-2">
            ...
            <ConfidenceBadge level={(evidenceLevel as any) || 'L1'} size="sm" />
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 mt-3 text-xs text-gray-500">
          ...
          {onFavoriteToggle && (
            <button
              ...
              className={`transition-colors flex items-center gap-1 ${
                isFavorite ? 'text-amber-400 hover:text-amber-300' : 'text-gray-500 hover:text-amber-400'
              }`}
            >
          ...
          <button
            onClick={handleShare}
            className={`transition-colors flex items-center gap-1 ${
              shareCopied ? 'text-emerald-400' : 'text-gray-500 hover:text-violet-400'
            }`}
        ...
        {dimensions && dimensions.length > 0 && (
          <div className="mt-4 bg-gray-800/30 rounded-lg p-3">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs text-gray-500">六维评分</span>
              <button
                  ...
                  return (
                    <div key={dim.dimension} className="flex flex-col">
                      <div className="flex items-center justify-between text-xs">
                        <span className="text-gray-500">{dim.dimension}</span>
