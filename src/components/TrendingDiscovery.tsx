...
const TrendingDiscovery: React.FC<TrendingDiscoveryProps> = ({ onEvaluateProject }) => {
  ...
  const [discoveryResults, setDiscoveryResults] = useState<ProjectRecommendation[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        unlisten = await listen<{ status: DiscoveryStatus; newDiscoveries: number }>('discovery_update', (event) => {
          ...
          if (event.payload.newDiscoveries > 0) {
            loadDiscoveryResults().then(() => {
              ...
              if (newCount > prevDiscoveryCount.current && prevDiscoveryCount.current > 0) {
                notifyDiscovery(
  ...
  const filteredDiscovery = selectedLang === 'All'
    ? discoveryResults
        ...
        <>
          ...
          {showSettings && (
            <div className={`mb-4 rounded-lg border p-4 ${
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-xs text-gray-400 mb-1">{t('starRange')}</label>
              ...
              <div className="flex items-center gap-2 mt-3">
                ...
                {permission === 'granted' && (
                  <span className="text-xs text-emerald-400">{t('notificationsEnabled')}</span>
                )}
              </div>
            </div>
          )}
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {project.grade && (
                        <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold ${
                          project.grade === 'S' ? 'bg-amber-500/20 text-amber-400' :
                          project.grade === 'A' ? 'bg-emerald-500/20 text-emerald-400' :
                          project.grade === 'B' ? 'bg-blue-500/20 text-blue-400' :
          ...
          {!discoveryRunning && discoveryResults.length === 0 && (
            <div className="text-center py-12 text-gray-400">
              ...
              <p className="text-xs text-gray-500 mt-1">{t('discoveryEmptySubHint')}</p>
            </div>
          )}

          {discoveryRunning && discoveryResults.length === 0 && (
            <div className="text-center py-12 text-gray-400">
              <svg className="w-12 h-12 mx-auto mb-3 animate-spin text-violet-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
              </svg>
              <p className="text-sm">{t('discoverySearching')}</p>
              <p className="text-xs text-gray-500 mt-1">{t('discoveryFirstTime')}</p>
            </div>
          )}
        </>
      )}
