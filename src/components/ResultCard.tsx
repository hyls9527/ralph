...
const RadarChart: React.FC<{
  ...
  const labelPoints = useMemo(
    () => dimensions.map((_, i) => getPoint(i, 1.15)),
    [n],
  );
...
// 轨道专属指标徽章
const TrackMetricBadge: React.FC<{
  track: string;
  project: ProjectRecommendation;
}> = ({ track, project }) => {
  if (track === 'neglected') {
    ...
    <div className="mt-3 bg-amber-900/20 border border-amber-500/30 rounded-lg p-3 animate-fade-in">
      ...
      <ul className="space-y-0.5">
        {warnings.slice(0, 3).map((w, i) => (
          <li
            key={i}
            className="text-xs text-amber-300/80 pl-5 relative before:content-['•'] before:absolute before:left-3 before:text-amber-500"
          >
            {w}
          </li>
        ))}
        {warnings.length > 3 && (
          <li className="text-xs text-amber-400/60 pl-5">
            +{warnings.length - 3} 条更多
          </li>
        )}
      </ul>
    </div>
...
// 维度子分数下钻
const DimensionDrillDown: React.FC<{
  dimension: {
    dimension: string;
    score: number;
    maxScore: number;
  ...
  return (
    <div className="border-t border-gray-700/50 pt-1.5 mt-1.5">
      <button
        ...
        className="text-xs text-gray-400 hover:text-gray-200 transition-colors flex items-center gap-1 w-full"
      >
        <svg
      ...
      <div className="p-5">
        ...
        <div className="flex items-center gap-4 mt-3 text-xs text-gray-400">
          ...
          <button
            ...
            className="ml-auto text-violet-400 hover:text-violet-300 transition-colors flex items-center gap-1"
          >
            详情
            <svg
              className="w-3 h-3"
        ...
        <div className="flex items-center gap-3 mt-4 flex-wrap">
          <div className="flex items-center gap-2">
            ...
            <span
              className={`text-xs font-semibold px-2 py-0.5 rounded border ${gradeColors[grade]}`}
            >
              {grade}级
            </span>
          </div>
            ...
            {radarView ? (
              ...
              <div className="grid grid-cols-3 gap-2">
                {dimensions.map((dim) => {
                  ...
                  const color =
                    dimensionColors[dim.dimension] || 'text-gray-400';
                  return (
        ...
        {decisionTrail && decisionTrail.length > 0 && (
          <div className="mt-2 flex items-center gap-2 text-xs text-gray-500">
            ...
            <span>
              ...
              {decisionTrail[0]?.before.toFixed(1)} →{' '}
              {decisionTrail[decisionTrail.length - 1]?.after.toFixed(1)}
            </span>
            {decisionTrail.some((s) => s.after < s.before) && (
              <span className="text-amber-400">
                ...
                {decisionTrail
                  ...
                  .reduce((acc, s) => acc + (s.before - s.after), 0)
                  .toFixed(1)}
                )
              </span>
            )}
                    ...
                    <span
                      className={`text-lg font-semibold ${
                        trustBadge.l2.keyMetrics.securityStatus === 'passed'
                          ? 'text-emerald-400'
                          : trustBadge.l2.keyMetrics.securityStatus ===
                              'warning'
                            ? 'text-amber-400'
                            : 'text-rose-400'
                      }`}
                  ...
                  <div className="space-y-1">
                    {trustBadge.l2.gateChecks.map((gate, i) => (
                      ...
                      >
                        ...
                        <span
                          className={
                            gate.passed ? 'text-emerald-400' : 'text-rose-400'
                          }
                        >
                          {gate.passed ? '通过' : '未通过'}
                        </span>
