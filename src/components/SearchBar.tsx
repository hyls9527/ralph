...
const SearchBar = function SearchBar({ ref: externalRef }: SearchBarProps) {
  ...
  const generateSuggestions = useCallback((input: string) => {
    ...
    if (trimmed) {
      ...
      historyCacheRef.current
        .filter(h => h.keyword.toLowerCase().includes(lowerInput) && h.keyword.toLowerCase() !== lowerInput)
        .slice(0, 5)
        .forEach(h => {
  ...
  useEffect(() => {
    ...
    return () => {
      if (externalRef && typeof externalRef !== 'function') {
        (externalRef as React.MutableRefObject<SearchBarHandle | null>).current = null;
      }
    };
