...
export function filterAndSort(
  ...
  const filtered = results.filter((p) => {
    if (criteria.trackFilter !== 'all' && p.track !== criteria.trackFilter)
      return false;
    if (
      criteria.languageFilter !== 'all' &&
      p.repo.language !== criteria.languageFilter
    )
      return false;
...
export function extractLanguages(results: ProjectRecommendation[]): string[] {
  ...
  results.forEach((p) => {
    if (p.repo.language) set.add(p.repo.language);
  });
