...
function createMockProject(overrides: Partial<ProjectRecommendation> = {}): ProjectRecommendation {
  return {
    repo: {
      owner: 'test', name: 'repo', fullName: 'test/repo', htmlUrl: '',
      description: null, stargazersCount: 100, forksCount: 10, openIssuesCount: 5,
      language: 'Rust', createdAt: '2023-01-01T00:00:00Z', updatedAt: '2024-12-01T00:00:00Z',
      pushedAt: '2024-12-01T00:00:00Z', license: null, size: 5000,
      hasWiki: false, hasIssuesEnabled: false, topics: [],
    },
    gateChecks: [], track: 'steady', neglectIndex: 3.0,
    dimensions: [
    ...
    totalScore: 77, grade: 'B', oneLiner: '', evidenceLevel: 'L1',
    trustBadge: { level: 2, l1: { status: 'recommended', icon: '✓', label: '推荐', color: 'emerald' } },
    vetoFlags: [], recommendationIndex: 38.5, confidenceTier: 'tier1-core', decisionTrail: [],
    ...overrides,
  };
}

type PartialProject = Partial<Omit<ProjectRecommendation, 'repo'>> & { repo?: Partial<ProjectRecommendation['repo']> };

function makeProject(name: string, opts: PartialProject = {}): ProjectRecommendation {
...
describe('useFilteredResults 纯函数测试', () => {
  beforeEach(() => {
    useAppStore.setState({ results: [], loading: { phase: 'idle', message: '' }, query: '', token: '' });
  ...
  describe('filterAndSort - 排序', () => {
    const projects = [
      makeProject('a/repo', { totalScore: 60, recommendationIndex: 30, repo: { stargazersCount: 5000, updatedAt: '2024-01-01T00:00:00Z' } }),
      makeProject('b/repo', { totalScore: 90, recommendationIndex: 135, repo: { stargazersCount: 1000, updatedAt: '2024-12-01T00:00:00Z' } }),
      makeProject('c/repo', { totalScore: 75, recommendationIndex: 60, repo: { stargazersCount: 500, updatedAt: '2024-06-01T00:00:00Z' } }),
    ];
  ...
  describe('paginate', () => {
    it('分页计算正确', () => {
      const projects = Array.from({ length: 25 }, (_, i) => makeProject(`test/repo-${i}`));
      const { totalResults, totalPages, hasNextPage, hasPrevPage } = paginate(projects, 1);
    ...
    it('超出范围页码自动修正', () => {
      const projects = Array.from({ length: 5 }, (_, i) => makeProject(`test/repo-${i}`));
  ...
  describe('extractLanguages', () => {
    it('提取所有唯一语言', () => {
      const projects = [
        makeProject('r1', { repo: { ...createMockProject().repo, language: 'Rust' } }),
        makeProject('r2', { repo: { ...createMockProject().repo, language: 'Python' } }),
        makeProject('r3', { repo: { ...createMockProject().repo, language: 'Rust' } }),
        makeProject('r4', { repo: { ...createMockProject().repo, language: null } }),
      ];
  ...
  describe('applyDimensionWeights', () => {
    it('应用自定义权重重新评分', () => {
      ...
      const weights: DimensionWeights = { quality: 100, maintenance: 0, practical: 0, documentation: 0, community: 0, security: 0 };
    ...
    it('权重为零时返回原始结果', () => {
      const project = createMockProject();
      const weights: DimensionWeights = { quality: 0, maintenance: 0, practical: 0, documentation: 0, community: 0, security: 0 };
      const result = applyDimensionWeights([project], weights);
    ...
    it('无 dimensionWeights 时返回原始结果', () => {
      const project = createMockProject();
      const result = applyDimensionWeights([project], { quality: 0, maintenance: 0, practical: 0, documentation: 0, community: 0, security: 0 });
