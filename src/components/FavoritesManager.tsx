...
const FavoritesManager: React.FC<FavoritesManagerProps> = ({ isLight }) => {
  ...
  const handleRemoveFavorite = useCallback(
    async (fullName: string) => {
      ...
      setFavoriteDetails((prev) =>
        prev.filter((f) => f.repo.full_name !== fullName),
      );
    },
    [toggleFavorite],
  );
  ...
  const activeFavorites = favoriteDetails.filter((f) =>
    isFavorite(f.repo.full_name),
  );
            ...
            >
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span
                    className={`font-medium truncate ${isLight ? 'text-gray-700' : 'text-gray-300'}`}
                  >
                    {item.repo.full_name}
                  </span>
                  <span
                    className={`font-bold ${gradeColors[item.grade] || 'text-gray-400'}`}
                  >
                    {item.grade}
                  </span>
                ...
                <div
                  className={`flex items-center gap-2 mt-0.5 ${isLight ? 'text-gray-400' : 'text-gray-400'}`}
                >
                  <span>{trackLabels[item.track] || item.track}</span>
                  <span>·</span>
                  <span>
                    {t('recommendationIndex')}{' '}
                    {item.recommendationIndex.toFixed(2)}
                  </span>
                  <span>·</span>
