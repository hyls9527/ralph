...
describe('EmptyState', () => {
  ...
  it('无结果且未搜索时显示初始提示', () => {
    const html = renderToString(<EmptyState hasResults={false} hasSearched={false} />);
    expect(html).toContain('开始搜索');
    expect(html).toContain('GitHub');
  });

  it('无结果且已搜索时显示无匹配提示', () => {
    const html = renderToString(<EmptyState hasResults={false} hasSearched={true} />);
    expect(html).toContain('未找到匹配的项目');
    expect(html).toContain('尝试调整');
  });

  it('提供 onClearFilters 时显示清除按钮', () => {
    const html = renderToString(<EmptyState hasResults={false} hasSearched={true} onClearFilters={vi.fn()} />);
