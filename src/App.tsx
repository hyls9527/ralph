...
function App() {
  const { results, query, loading, setToken, setLoading, setSearchResults } =
    useAppStore();
  ...
  const setSelectedDetailProject = useUiStore(
    (s) => s.setSelectedDetailProject,
  );
  ...
  const handleEvaluateFromTrending = useCallback(
    async (repo: ProjectRecommendation) => {
      setLoading({
        phase: 'searching',
        message: t('evaluatingProject', { name: repo.repo.fullName }),
      });
      try {
        const response = await tauri.searchAndEvaluate(
        ...
        setLoading({
          phase: 'error',
          message: `${t('evaluateFailed')}: ${String(error)}`,
        });
      }
    },
  ...
  const runBatch = async () => {
    ...
    setLoading({
      phase: 'evaluating',
      message: t('batchEvaluating', { count }),
    });
    try {
      const response = await tauri.batchEvaluate(
        query.trim(),
        Math.min(Math.max(count, 10), 100),
      );
      setSearchResults(response.results);
      setShowSettings(false);
      ...
      setLoading({
        phase: 'error',
        message: `${t('batchFailed')}: ${String(err)}`,
      });
  ...
  const resumeBatch = async (sessionId: string) => {
    ...
    try {
      ...
      setLoading({
        phase: 'error',
        message: t('resumeFailed', { error: String(err) }),
      });
    }
  };
  ...
  const selectedProjectsList = filteredResults.filter((p) =>
    selectedProjects.includes(p.repo.fullName),
  );
  ...
  return (
    <div
      className={`min-h-screen ${isDark ? 'bg-gray-950 text-gray-100' : 'bg-gray-50 text-gray-900'} transition-colors duration-200`}
    >
      <Header />

      <main className="max-w-4xl mx-auto px-6 py-8">
        <div id="ralph-search-input">
        ...
        {showTrending && (
          <div className="mb-6 animate-fade-in">
            <Suspense fallback={<LazyLoadingFallback />}>
              <TrendingDiscovery
                onEvaluateProject={handleEvaluateFromTrending}
              />
            </Suspense>
          </div>
        )}
        ...
        {(loading.phase === 'searching' || loading.phase === 'evaluating') &&
          results.length === 0 && (
            <ResultsSkeleton count={3} isLight={!isDark} />
          )}
        ...
        {(loading.phase === 'searching' || loading.phase === 'evaluating') && (
          <div
            role="status"
            aria-live="polite"
            className={`rounded-xl p-4 mb-4 border ${!isDark ? 'bg-blue-50 border-blue-200' : 'bg-blue-900/10 border-blue-700/30'}`}
          >
            <div className="flex items-center gap-3">
              ...
              <p
                className={`text-sm font-medium flex-1 ${!isDark ? 'text-gray-900' : 'text-gray-200'}`}
              >
                {loading.phase === 'searching'
                  ? t('searching')
                  : loading.phase === 'evaluating'
        ...
        {compareMode && selectedProjectsList.length > 0 && (
          <Suspense fallback={<LazyLoadingFallback />}>
            <ComparisonPanel
              ...
              onRemoveProject={toggleProjectSelection}
            />
          </Suspense>
        )}
        {results.length > 0 && (
          ...
          <div>
            <div className="flex items-center justify-between mb-4 gap-3 flex-wrap">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={() =>
                    useUiStore.getState().setCompareMode(!compareMode)
                  }
            ...
            {showFilters && (
              ...
              >
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  <div>
                    <label
                      className={`text-xs block mb-1 ${!isDark ? 'text-gray-600' : 'text-gray-400'}`}
                    >
                      {t('trackLabel')}
                    </label>
                    <select
                      ...
                      onChange={(e) =>
                        useFilterStore
                          .getState()
                          .setTrackFilter(e.target.value as any)
                      }
                    ...
                    >
                      ...
                      <option value="neglected">{t('neglected')}</option>
                      <option value="high-star">{t('highStar')}</option>
                      <option value="steady">{t('steady')}</option>
                    </select>
                  </div>
                  <div>
                    <label
                ...
                <div
                  className={`mt-3 pt-3 border-t flex items-center gap-2 ${!isDark ? 'border-gray-200' : 'border-gray-700'}`}
                >
                  <span
                    className={`text-xs ${!isDark ? 'text-gray-600' : 'text-gray-400'}`}
                  >
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      useFilterStore.getState().setSortBy(e.target.value as any)
                    }
                  ...
                  >
                    <option value="recommendationIndex">
                      {t('recommendationIndex')}
                    </option>
                    <option value="score">{t('totalScoreSort')}</option>
                    <option value="stars">{t('starCount')}</option>
                  </select>
                  <button
                    onClick={() =>
                      useFilterStore
            ...
            {filteredResults.length > 30 && !compareMode ? (
              <div
                className={`rounded-xl border overflow-hidden ${!isDark ? 'bg-white/60 border-gray-200 hover:border-violet-400/40' : 'bg-gray-900/60 border-gray-800 hover:border-violet-500/40'} backdrop-blur`}
              >
                <VirtualList
              ...
              <div className="space-y-4">
                {paginatedResults.map((project) => (
                  <div
                    ...
                    className={`rounded-xl p-0 transition-all duration-200 ${!isDark ? 'bg-white/60 border-gray-200 hover:border-violet-400/40' : 'bg-gray-900/60 border-gray-800 hover:border-violet-500/40'} backdrop-blur border overflow-hidden`}
                  >
                    {compareMode && (
                      <button
                        onClick={() =>
                          toggleProjectSelection(project.repo.fullName)
                        }
                      ...
                      >
                        {isSelected(project.repo.fullName) && (
                          <svg
                            className="w-3 h-3"
                            fill="currentColor"
                            viewBox="0 0 20 20"
                    ...
                    <div className={compareMode ? 'ml-8' : ''}>
                      <ResultCard
                        project={project}
                        onDetailClick={() => setSelectedDetailProject(project)}
                        isFavorite={isFavorite(project.repo.fullName)}
                        onFavoriteToggle={() =>
                          toggleFavorite(project.repo.fullName, project)
                        }
                      />
                    </div>
                  </div>
            ...
            {totalPages > 1 && filteredResults.length <= 30 && (
              <div className="flex items-center justify-center gap-2 mt-6">
                <button
                  ...
                  className={`text-xs px-2 py-1 rounded border disabled:opacity-40 ${!isDark ? 'border-gray-300 text-gray-600 hover:bg-gray-100' : 'border-gray-700 text-gray-400 hover:bg-gray-800'}`}
                >
                  {t('prevPage')}
                </button>
                {Array.from({ length: totalPages }, (_, i) => i + 1).map(
                  (page) => (
                    <button
                      ...
                      className={`w-8 h-8 text-xs rounded-lg transition-colors ${page === currentPage ? 'bg-violet-600 text-white' : !isDark ? 'text-gray-600 hover:bg-gray-100' : 'text-gray-400 hover:bg-gray-800'}`}
                    >
                      {page}
                    </button>
                  ),
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex-1 text-sm py-2 bg-violet-600 text-white rounded-lg hover:bg-violet-500 focus-visible:ring-2 focus-visible:ring-violet-400"
            >
              {t('save')}
            </button>
            <button
              onClick={clearCache}
              className={`text-sm px-4 py-2 rounded-lg border ${!isDark ? 'border-gray-300 text-gray-600 hover:bg-gray-100' : 'border-gray-700 text-gray-400 hover:bg-gray-800'} focus-visible:ring-2 focus-visible:ring-violet-400`}
            >
              {t('clearCache')}
            </button>
          </div>
          <div
            className={`border-t pt-4 ${!isDark ? 'border-gray-200' : 'border-gray-700'}`}
          >
            <h4
              className={`text-sm font-semibold mb-2 ${!isDark ? 'text-gray-700' : 'text-gray-300'}`}
            >
              {t('batchEvaluate')}
            </h4>
            <div className="flex gap-2">
              ...
              <button
                onClick={runBatch}
                className="flex-1 text-sm py-1.5 bg-emerald-600 text-white rounded-lg hover:bg-emerald-500 focus-visible:ring-2 focus-visible:ring-emerald-400"
              >
                {t('startBatch')}
              </button>
            </div>
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-gray-500">
                          {session.processed}/{session.totalRepos} ·{' '}
                      ...
                      <button
                        onClick={() =>
                          deleteBatchSessionHandler(session.sessionId)
                        }
          ...
          >
            ...
            <p
              className={`text-xs mb-3 ${!isDark ? 'text-amber-600' : 'text-amber-400'}`}
            >
              {t('weightsWarning')}
            </p>
            <div className="space-y-2">
              {Object.entries(dimensionWeights).map(([key, value]) => (
                <div key={key} className="flex items-center gap-2">
                  <label
                    htmlFor={`weight-${key}`}
                    className={`text-xs w-16 ${!isDark ? 'text-gray-600' : 'text-gray-400'}`}
                  >
                    {{
                      quality: t('qualityDim'),
                      maintenance: t('maintenanceDim'),
                      practical: t('practicalDim'),
                      documentation: t('documentationDim'),
                      community: t('communityDim'),
                      security: t('securityDim'),
                    }[key] || key}
                  </label>
                  <input
                    type="range"
                  ...
                  />
                  <span
                    className={`text-xs w-8 text-right ${!isDark ? 'text-gray-700' : 'text-gray-300'}`}
                  >
                    {value}
                  </span>
                </div>
            ...
            <button
              ...
              className={`flex-1 text-xs py-2 rounded-lg border mt-3 ${!isDark ? 'border-gray-300 text-gray-600 hover:bg-gray-100' : 'border-gray-700 text-gray-400 hover:bg-gray-800'} focus-visible:ring-2 focus-visible:ring-violet-400`}
            >
              {t('resetDefault')}
            </button>
          </div>
