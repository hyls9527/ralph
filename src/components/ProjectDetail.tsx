...
const RadarChart: React.FC<{
  dimensions: ProjectRecommendation['dimensions'];
  size?: number;
}> = ({ dimensions, size = 200 }) => {
  const cx = size / 2;
  const cy = size / 2;
  ...
  const dataPoints = dimensions.map((dim, i) =>
    getPoint(i, dim.score, dim.maxScore),
  );
...
const ActivityTimeline: React.FC<{ project: ProjectRecommendation }> = ({
  project,
}) => {
  ...
  const events = useMemo(() => {
    const items: {
      ...
      label: string;
      detail: string;
      type: 'created' | 'updated' | 'pushed' | 'eval';
    }[] = [];
      ...
      {events.map((event, i) => (
        <div key={i} className="flex gap-3">
          <div className="flex flex-col items-center">
            <div
              className={`w-2.5 h-2.5 rounded-full border-2 ${typeColors[event.type]}`}
            />
            {i < events.length - 1 && (
              <div className="w-px flex-1 bg-gray-700 my-0.5" />
            )}
          </div>
...
const SecuritySummary: React.FC<{ project: ProjectRecommendation }> = ({
  project,
}) => {
  const { trustBadge, vetoFlags, gateChecks } = project;

  const securityGate = gateChecks.find(
    (g) => g.gate.includes('安全') || g.gate.includes('Security'),
  );
  const licenseGate = gateChecks.find(
    (g) => g.gate.includes('License') || g.gate.includes('协议'),
  );

  const securityScore =
    project.dimensions.find((d) => d.dimension === '安全')?.score ?? 0;
  const securityMax =
    project.dimensions.find((d) => d.dimension === '安全')?.maxScore ?? 20;
    ...
    <div className="space-y-3">
      <div className="bg-gray-800/30 rounded-lg p-4">
        <div className="flex items-center justify-between mb-2">
          ...
          <span
            className={`text-sm font-semibold ${securityPct >= 70 ? 'text-emerald-400' : securityPct >= 40 ? 'text-amber-400' : 'text-rose-400'}`}
          >
      ...
      <div className="grid grid-cols-2 gap-2">
        <div className="bg-gray-800/30 rounded-lg p-3">
          ...
          <span
            className={`text-sm font-medium ${licenseGate?.passed ? 'text-emerald-400' : 'text-rose-400'}`}
          >
        ...
        <div className="bg-gray-800/30 rounded-lg p-3">
          ...
          <span
            className={`text-sm font-medium ${securityGate?.passed ? 'text-emerald-400' : 'text-rose-400'}`}
          >
        ...
        <div className="bg-gray-800/30 rounded-lg p-3">
          ...
          <span
            className={`text-sm font-medium ${
              trustBadge.l1.status === 'recommended'
                ? 'text-emerald-400'
                : trustBadge.l1.status === 'caution'
                  ? 'text-amber-400'
                  : 'text-rose-400'
            }`}
          >
        ...
        <div className="bg-gray-800/30 rounded-lg p-3">
          ...
          <span
            className={`text-sm font-medium ${vetoFlags.length === 0 ? 'text-emerald-400' : 'text-rose-400'}`}
          >
      ...
      {vetoFlags.length > 0 && (
        <div className="space-y-1.5">
          {vetoFlags.map((flag, i) => (
            <div
              key={i}
              className="flex items-start gap-2 bg-rose-900/20 border border-rose-500/30 rounded-lg p-2.5"
            >
              <span className="text-xs text-rose-400 flex-shrink-0 mt-0.5">
                ⚠️
              </span>
...
const ProjectDetail: React.FC<ProjectDetailProps> = ({ project, onClose }) => {
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-3xl max-h-[90vh] bg-gray-900 rounded-2xl shadow-2xl border border-gray-800 overflow-hidden animate-fade-in"
      ...
      >
        <div className="p-6 border-b border-gray-800">
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <span
                className={`text-xs font-semibold px-2 py-1 rounded border ${gradeColors[grade]}`}
              >
                {grade}
                {t('gradeSuffix')}
              </span>
          ...
          <div className="flex items-center gap-2 mt-3 flex-wrap">
            ...
            {track === 'neglected' && neglectIndex !== undefined && (
              <span className="text-xs px-2 py-0.5 bg-emerald-900/20 text-emerald-300 rounded border border-emerald-500/40">
                {t('neglectIndex')}: {neglectIndex.toFixed(1)}
              </span>
            )}
            {track === 'high-star' && valueDensity !== undefined && (
              <span className="text-xs px-2 py-0.5 bg-amber-900/20 text-amber-300 rounded border border-amber-500/40">
                {t('valueDensity')}: {valueDensity.toFixed(2)}
              </span>
            )}
            {track === 'steady' && steadyState !== undefined && (
              <span className="text-xs px-2 py-0.5 bg-blue-900/20 text-blue-300 rounded border border-blue-500/40">
                {t('steadyState')}: {steadyState.toFixed(2)}
              </span>
            )}
          </div>
        </div>
        ...
        <div className="p-6 overflow-y-auto max-h-[60vh]">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-gray-800/30 rounded-lg p-4">
                  ...
                  <span className="text-lg font-semibold text-emerald-400">
                    {trustBadge.l2?.keyMetrics.qualityScore ?? '-'}
                  </span>
                </div>
                <div className="bg-gray-800/30 rounded-lg p-4">
                  <span className="text-xs text-gray-400 block">
                  ...
                  <span className="text-lg font-semibold text-blue-400">
                    {trustBadge.l2?.keyMetrics.maintenanceScore ?? '-'}
                  </span>
                </div>
                <div className="bg-gray-800/30 rounded-lg p-4">
                  <span className="text-xs text-gray-400 block">
                  ...
                  <span
                    className={`text-lg font-semibold ${
                      trustBadge.l2?.keyMetrics.securityStatus === 'passed'
                        ? 'text-emerald-400'
                        : trustBadge.l2?.keyMetrics.securityStatus === 'warning'
                          ? 'text-amber-400'
                          : 'text-rose-400'
                    }`}
                  >
              ...
              {repo.topics && repo.topics.length > 0 && (
                <div className="flex flex-wrap gap-1.5">
                  {repo.topics.map((topic) => (
                    <span
                      key={topic}
                      className="text-xs px-2 py-0.5 bg-violet-900/20 text-violet-300 rounded-full border border-violet-700/30"
                    >
                      {topic}
                    </span>
                  ))}
                  ...
                  return (
                    ...
                    >
                      <div
                        ...
                        style={{ color: dimensionStrokeColors[dim.dimension] }}
                      >
                        {dim.score.toFixed(1)}
                      </div>
              ...
              {dimensions.map((dim) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="w-full h-2 bg-gray-700 rounded-full overflow-hidden">
                      <div
                        className={`h-full rounded-full ${dimensionColors[dim.dimension] || 'bg-gray-500'}`}
                        style={{ width: `${pct}%` }}
                      />
                    </div>
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className="flex-1 h-1 bg-gray-700 rounded-full overflow-hidden">
                                <div
                                  className={`h-full rounded-full ${subPct >= 70 ? 'bg-emerald-500' : subPct >= 40 ? 'bg-amber-500' : 'bg-rose-500'}`}
                                  style={{ width: `${subPct}%` }}
                                />
                              </div>
          ...
          {activeTab === 'activity' && <ActivityTimeline project={project} />}

          {activeTab === 'security' && <SecuritySummary project={project} />}
              ...
              {gateChecks.map((gate, i) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={`text-sm font-medium ${gate.passed ? 'text-emerald-400' : 'text-rose-400'}`}
                    >
              ...
              {vetoFlags && vetoFlags.length > 0 ? (
                <div className="space-y-2">
                  {vetoFlags.map((w, i) => (
                    <div
                      key={i}
                      className="flex items-start gap-3 bg-amber-900/20 border border-amber-500/30 rounded-lg p-3"
                    >
                      <svg
