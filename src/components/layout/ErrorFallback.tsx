...
export function ErrorFallback({ error, onRetry }: ErrorFallbackProps) {
  ...
  return (
    <div className={`min-h-screen ${isLight ? 'bg-gray-50 text-gray-900' : 'bg-gray-950 text-gray-100'} flex items-center justify-center`}>
      <div className="text-center p-8 max-w-md">
        ...
        <p className={`text-sm mb-6 ${isLight ? 'text-gray-500' : 'text-gray-400'}`}>{error}</p>
        {onRetry && (
          <button
            onClick={onRetry}
