...
interface TrendingDiscoveryProps {
  onEvaluateProject: (repo: any) => void;
}
...
const TrendingDiscovery: React.FC<TrendingDiscoveryProps> = ({ onEvaluateProject }) => {
  ...
  const fetchTrending = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await invoke<any>('get_trending_repos');
    ...
    <div className="bg-gray-900/60 backdrop-blur rounded-xl border border-gray-800 p-6">
          ...
          {projects.map((project, i) => (
            ...
            >
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-500 mt-0.5 truncate">{project.description || '无描述'}</p>
                <div className="flex items-center gap-3 mt-1 text-xs text-gray-500">
                  ...
                  <span className="flex items-center gap-1">
                    ...
                    {(project.stargazers_count || 0).toLocaleString()}
                  </span>
                </div>
              </div>
              <button
      ...
      {!loading && projects.length === 0 && !error && (
        <div className="text-center py-12 text-gray-500">
          ...
          <p className="text-sm">点击「获取 Trending」发现热门项目</p>
        </div>
      )}
    </div>
  );
