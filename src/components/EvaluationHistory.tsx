...
const EvaluationHistory: React.FC<EvaluationHistoryProps> = ({ repoFullName, isLight }) => {
  ...
  const fetchTrend = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await invoke<ScoreTrendPoint[]>('get_score_trend', { repoFullName });
      setTrend(data);
    } catch (e: any) {
      setError(String(e));
    } finally {
  ...
  if (trend.length === 0) {
    return (
      <div className={`text-sm py-8 text-center ${isLight ? 'text-gray-500' : 'text-gray-600'}`}>
        暂无评估历史
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      <div className="mt-4 space-y-2 max-h-64 overflow-y-auto">
        {trend.slice().reverse().slice(0, 10).map((t, i) => (
          <div
            key={i}
          ...
          >
            <div className="flex items-center gap-3">
              <span className={`font-semibold px-1.5 py-0.5 rounded border ${gradeColors[t.grade]}`}>
                {t.grade}级
              </span>
              <span className={isLight ? 'text-gray-700' : 'text-gray-300'}>
                {t.score.toFixed(1)}分
              </span>
              <span className={isLight ? 'text-gray-500' : 'text-gray-600'}>
                {trackLabels[t.track] || t.track}轨道
              </span>
            </div>
            <span className={isLight ? 'text-gray-400' : 'text-gray-600'}>
              {new Date(t.evaluatedAt).toLocaleString('zh-CN')}
            </span>
          </div>
        ))}
      </div>
